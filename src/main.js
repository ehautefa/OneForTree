...
async function launchGame() {
  ...
  playerDestination.setPlayerDestination = (val) => {	
	...
	socket.emit(
        "move",
        { position: { x: -val.x, y: -val.y }, uuid: user.id },
        ({ position: { x, y } }) => {
          playerDestination.x = val.x * 30 + offset.x;
          playerDestination.y = val.y * 30 + offset.y;
  ...
  document.addEventListener(
    ...
    (event) => {
      var name = event.key;
      if (name == "ArrowRight") user.x--;
      if (name == "ArrowDown") user.y--;
      if (name == "ArrowUp") user.y++;
      if (name == "ArrowLeft") user.x++;
      playerDestination.setPlayerDestination({ x:user.x, y:user.y });
    },
    false
  );
